# Indexed content and security considerations

This index is built entirely from public source, but "public" doesn't automatically mean "safe to
re-publish verbatim." This doc explains exactly what `scripts/jeap-index-all.sh`,
`scripts/jeap-index.sh`, `scripts/jeap-stage-docs.sh`, and `project-rag`'s file walker sweep into
the index, what's filtered out, and what to check before pointing a new publicly-reachable
consumer (e.g. `jeap-mcp-service`) at this image.
...
## Before exposing a new downstream consumer publicly
