...
`scripts/jeap-index-all.sh` indexes three sets of repos and calls `scripts/jeap-index.sh` once per repo: JEAP infra repos (Bitbucket project `JEAP`, `scm/jeap`) and JME example repos (Bitbucket project `BIT_JME`, `scm/bit_jme`) are **auto-discovered from the Bitbucket API** (archived/excluded repos dropped), plus a **static list** of public OSS repos under `github.com/jeap-admin-ch` (`GITHUB_REPOS`). Stops at the first failure (`set -euo pipefail`). See the auto-discovery details below.

- **JEAP repos** are indexed with `--strip-tests --exclude-docs`: `--strip-tests` deletes `src/test` trees before indexing (those tests are rarely relevant to agents writing applications *with* jEAP), and `--exclude-docs` keeps the repo-root `docs/` out of the per-repo index (deduplication — see the `jeap-docs` section below).
...
- **GitHub OSS repos** are indexed with `--strip-tests --exclude-docs`, same as the JEAP infra repos. These are public, so the clone needs no credentials.
...
3. **Rewrite doc links** — rewrite `jeap-admin-ch` GitHub links in Markdown to index-local paths via `scripts/jeap-rewrite-doc-links.sh`. Always attempted (both modes); the helper no-ops when `INDEXED_SLUGS` is unset and is idempotent, so it is safe to run on an already-rewritten staged corpus.
